...
def register_api_get_capstone_route(app: FastAPI):
    ...
    def get_project(project_id: int, db: Session = Depends(get_db)):
        p = db.execute(
            text("SELECT id, title, year, abstract, filename, sha256, course, host, doc_type FROM projects WHERE id=:pid"),
            {"pid": project_id}
        ).fetchone()
        if not p: raise HTTPException(404, "Not found")
        pid, title, year, abstract, filename, sha, course, host, doc_type = p
        ...
        return {
            "id": pid, "title": title, "year": year, "abstract": abstract,
